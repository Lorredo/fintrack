...
func (s *BudgetService) List(ctx context.Context, userID, targetDate string) ([]models.Budget, error) {
	...
	for rows.Next() {
		var b models.Budget
		if err := rows.Scan(&b.ID, &b.UserID, &b.AccountID, &b.Category, &b.Amount, &b.PeriodType, &b.StartDate, &b.EndDate, &b.CreatedAt, &b.UpdatedAt, &b.Spent); err != nil {
			return nil, err
		}
...
func (s *BudgetService) Get(ctx context.Context, id, userID string) (*models.Budget, error) {
	...
	err := s.db.QueryRow(
		ctx,
		`SELECT id, user_id, account_id, category, amount, period_type, TO_CHAR(start_date, 'YYYY-MM-DD'), TO_CHAR(end_date, 'YYYY-MM-DD'), created_at, updated_at
		 FROM budgets WHERE id = $1 AND user_id = $2`,
		id, userID,
	).Scan(&b.ID, &b.UserID, &b.AccountID, &b.Category, &b.Amount, &b.PeriodType, &b.StartDate, &b.EndDate, &b.CreatedAt, &b.UpdatedAt)
...
func (s *BudgetService) Create(ctx context.Context, userID string, req models.CreateBudgetRequest) (*models.Budget, error) {
	...
	err := s.db.QueryRow(
		ctx,
		`INSERT INTO budgets (user_id, account_id, category, amount, period_type, start_date, end_date)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)
		 RETURNING id, user_id, account_id, category, amount, period_type, TO_CHAR(start_date, 'YYYY-MM-DD'), TO_CHAR(end_date, 'YYYY-MM-DD'), created_at, updated_at`,
		userID, req.AccountID, req.Category, req.Amount, req.PeriodType, req.StartDate, req.EndDate,
	).Scan(&b.ID, &b.UserID, &b.AccountID, &b.Category, &b.Amount, &b.PeriodType, &b.StartDate, &b.EndDate, &b.CreatedAt, &b.UpdatedAt)
...
func (s *BudgetService) Update(ctx context.Context, id, userID string, req models.UpdateBudgetRequest) (*models.Budget, error) {
	...
	query := `UPDATE budgets SET
		account_id = COALESCE(NULLIF($1, '')::UUID, account_id),
	...
	err := s.db.QueryRow(
		ctx, query,
		req.AccountID, req.Category, req.Amount, req.PeriodType, req.StartDate, req.EndDate, id, userID,
	).Scan(&b.ID, &b.UserID, &b.AccountID, &b.Category, &b.Amount, &b.PeriodType, &b.StartDate, &b.EndDate, &b.CreatedAt, &b.UpdatedAt)
